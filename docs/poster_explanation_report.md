...
넷째, 고성능 예측모형을 전면에 내세우기보다 설명 가능한 점검 우선순위 지수를 별도로 구성하였다. 우선순위 지수는 행정처분 발생률 분위점수 70%, 소비자물가지수 분위점수 20%, 기후부담 분위점수 10%를 결합하여 산출하였다. 이는 통계적 인과모형이 아니라, 지자체가 점검 후보를 정렬하기 위한 운영형 지표로 활용하기 위한 것이다.
...
| 점검 우선순위 지수 | 발생률 70% + CPI 20% + THI 10% | 예측모형보다 설명 가능한 운영형 지표로 활용 |
...
다만 전체 로지스틱 회귀모형에서는 총 소비자물가지수 1표준편차 증가 시 핫스팟 오즈가 약 39% 증가하는 방향을 보였다(OR=1.39, p=0.090). 이는 5% 유의수준의 확정적 인과 증거는 아니지만, 10% 유의수준에서 관찰되는 탐색적 신호로서 포스터의 핵심 문제의식인 “경제적 압박과 위생 취약 가능성”을 제기하는 근거로 활용할 수 있다.
...
LightGBM 분류모형도 실험했으나, 검증 AUC는 0.537이었다. 또한 다수 클래스 기준선의 accuracy가 0.785로 LightGBM accuracy 0.759보다 높았다. 따라서 본 연구에서는 LightGBM을 포스터의 핵심 분석 결과로 사용하지 않고, 설명 가능한 점검 우선순위 지수를 대시보드 활용 전략으로 제시하였다.
...
넷째, 대시보드 기반 점검 우선순위 체계를 구축할 수 있다. 시도-월별 행정처분 발생률, 경제 압박 지표, 기후 조건, 상권 규모를 결합하면 지자체가 제한된 점검 인력을 더 효율적으로 배분할 수 있다. 본 연구에서는 예측 성능이 낮은 복잡한 AI 모델 대신, 행정처분 발생률을 중심으로 경제·기후 점수를 보조 반영하는 설명 가능한 점검 우선순위 지수를 제안하였다.

![점검 우선순위 지수 산출](../output/eda_plots/stat_04_priority_index_components.png)

위 그림은 점검 우선순위 지수의 가중치 구조와 상위 후보 시도-월을 함께 보여준다. 행정처분 발생률을 가장 큰 비중으로 두고, 소비자물가지수와 기후부담 점수를 보조적으로 반영하여 지자체가 점검 후보를 정렬할 수 있도록 구성하였다.

| 활용 영역 | 기존 방식 | 제안 방식 | 기대효과 |
| --- | --- | --- | --- |
| 점검 대상 선정 | 절대 행정처분 건수 중심 | 음식점 수 대비 행정처분 발생률 중심 | 대도시 규모 착시 완화 |
| 점검 시점 판단 | 사후 적발 후 대응 | 경제·기후 신호 기반 사전 검토 | 예방 중심 관리 가능 |
