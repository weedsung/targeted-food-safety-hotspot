...
![분석 프로세스 시각화](../output/eda_plots/advanced_03_correlation_heatmap.png)

위 그림은 독립변수 간 상관관계를 확인하기 위한 시각화 자료다. 특히 `CPI_TOTAL`과 `CPI_AGRI`가 같은 물가 계열 변수로 비교적 높은 상관을 보이므로, 결과 해석 시 다중공선성 가능성을 함께 검토하였다.
