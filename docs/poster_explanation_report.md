...
넷째, 고성능 예측모형을 전면에 내세우기보다 설명 가능한 점검 우선순위 지수를 별도로 구성하였다. 우선순위 지수는 행정처분 발생률 분위점수 70%, 소비자물가지수 분위점수 20%, 기후부담 분위점수 10%를 결합하여 산출하였다. 이는 통계적 인과모형이 아니라, 지자체가 점검 후보를 정렬하기 위한 운영형 지표로 활용하기 위한 것이다. 해당 가중치는 통계적 최적화 결과가 아닌 정책 시뮬레이션용 초기값이며, 향후 전문가 의견과 현장 검증을 통해 조정할 수 있다.
...
3. **기후·상권 조건과 결합한 복합 신호 활용**  
   월별 기후조건, 지역별 상권 규모, 지역별 과거 행정처분률을 결합해 점검 우선순위를 설정한다.
...
> 경제적 압박은 식품접객업 행정처분 취약성과 연결될 가능성을 보였으며, 식품위생 점검은 절대 위반 건수가 아니라 월별 경제·기후 조건과 지역별 상권 규모 및 행정처분 발생률이 함께 변화하는 시도-월을 중심으로 설계할 필요가 있다.
